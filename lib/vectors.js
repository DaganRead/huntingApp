...
var Bone = function (v1, v2, bone) {
  ...
  if (bone) {
    ...
    this.angle = this.toVector().angle(this.parentbone.toVector())

  }
 
}

Bone.prototype.toVector = function () {
  var bonevec = new Vector({
    x: this.v2.x-this.v1.x,
    y: this.v2.y-this.v1.y,
    z: this.v2.z-this.v1.z,
  })
  return bonevec
}
...
Bone.prototype.update = function() {
  ...
  if (this.parentbone != 0) {
    var thisvec = this.toVector();
    thisvec.normalize();


    var parentb =  this.parentbone.toVector()
    parentb.scale(-1)
    parentb.normalize();
 

    thisvec.cross(parentb)    
    thisvec.normalize();


    if (this.rotationaxis) {
      var angle = thisvec.angle(this.rotationaxis)  
      if (isNaN(angle)) {
   ...
   this.angle = this.toVector().angle(this.parentbone.toVector()) * this.convex;
   var temp = this.angle;
   temp = temp / Math.PI;
   temp = (temp * 180) + this.servoOffset;
   

   this.servo = temp;
   if (this.servo <= 0) { this.servoOffset += 0 - this.servo}
   if (this.servo >= 180) { this.servoOffset += 180 - this.servo}
  }
