...
/* Start the server */
 ...
 /*sudo
// validation rule for username: should be one or more digits
		...
		db.users.find({'username': newuser.username}, function(err, users) {
			if ( err || !users) { 
				...
				if (users.length == 0) {
					...
					newuser.name = "";
					newuser.surname = "";
					newuser.avatar = "../uploads/avatars/default.svg";
					newuser.bio = "";
					newuser.location = "";
					newuser.website = "";
					newuser.projects = [];
...
/*	Restrict communication	*/
			...
			client.get(encryptedTokenhex, function(err, userData) {
		    	if (err) {
					...
					var newData = {
						...
						_id : returnData._id,
						avatar : data.avatar,
						levelAuthority : returnData.levelAuthority,
					    name: returnData.name,
					    surname: returnData.surname,
					    bio: returnData.bio,
					    location: returnData.location,
					    email: returnData.email,
					    website: returnData.website,
					    projects : returnData.projects
					};
							client.set(encryptedTokenhex, JSON.stringify(newData), function(err, reply) {
						        if (err) {
						        	console.log('token not sent or stored!'+err);
						        }else if (reply) {
						            client.expire(encryptedTokenhex, 86400, function(error, success) {
						...
						tokenGenerator.createToken(function (error, token) {
							if (error === null) {
									var project = {
							            ...
							            startDate: new Date(),
							            viewCount : 0,
							            projectCreatives:[returnData.username]
						            }; 
						            console.log(project);
								db.users.update(
								...
								var tempArray = returnData.projects;
									        ...
									        if (err) {
									            ...
									            client.expire(encryptedTokenhex, 86400, function(error, success) {
									                if (error) {
									                    ...
									                    restricted.to(socket.id).emit('recieve portfolioUpdate', project);
								...
								console.log('requested error project token');
								public.to(socket.id).emit('request error');
							}
						});
		    	};
	...
	socket.on('request projectRemove', function(data){
			...
			client.get(encryptedTokenhex, function(err, userData) {
		    	if (err) {
		    		...
		    		var returnData = JSON.parse(userData);
								...
								db.users.update(
								   {username:returnData.username},{ $pull: {
								      projects: { id: data.project.id }
								   }}
								);	
								var tempArray = returnData.projects;

								returnData.projects.every(function(element, index, array) {
									        ...
									        if (err) {
									            ...
									            client.expire(encryptedTokenhex, 86400, function(error, success) {
									                if (error) {
									                    ...
									                    restricted.to(socket.id).emit('recieve blab', tempArray);
		...
		socket.on('request portfolioFilesUpdate', function(data) {
			var encryptedToken = scrypt.crypto_scrypt(scrypt.encode_utf8(secret), scrypt.encode_utf8(data.token), 128, 8, 1, 32);
			var encryptedTokenhex = scrypt.to_hex(encryptedToken);
			//check the auth data sent by the client
			client.get(encryptedTokenhex, function(err, userData) {
		    	if (err) {
					...
					db.users.update(
						{"projects.id":data.project.id},
						{$set: {'projects.$.files': data.project.files } }
					);
					var tempArray = returnData.projects;

					returnData.projects.every(function(element, index, array) {
						if (element.id === data.project.id) {
							tempArray[index].files = data.project.files;
							return false;
						} else{
							return true;
						};
			...
			console.log("portfolioProject files updated");
			//restricted.to(socket.id).emit('recieve project', project);
		});
		
		socket.on('request portfolioCardUpdate', function(data) {
			...
			client.get(encryptedTokenhex, function(err, userData) {
		    		...
		    		var returnData = JSON.parse(userData);
						tokenGenerator.createToken(function (error, token) {
							if (error === null) {
								db.users.update(
									{"projects.id":data.project.id},
									{$set: {'projects.$.title': data.project.title, 'projects.$.description': data.project.description } }
								);
								var tempArray = returnData.projects;
									returnData.projects.every(function(element, index, array) {
										if (element.id === data.project.id) {
										    console.log(index);
										    tempArray[index].title = data.project.title;
										    tempArray[index].description = data.project.description;
										    return false;
										  } else{
										    return true;
										  };
									        ...
									        if (err) {
									            ...
									            client.expire(encryptedTokenhex, 86400, function(error, success) {
									                if (error) {
									                    ...
									                    //restricted.to(socket.id).emit('recieve portfolioUpdate', data.project);
				        								console.log('portfolio updated');	
									                }
									                else {
									                    console.log(new Error('Expiration not set on redis'));
		  	...
		  	//finds users in the database that have the same username already
			console.log("portfolioProject card updated");
			//restricted.to(socket.id).emit('recieve project', project);
		});
		socket.on('request projectComplete', function(data) {
		console.log(data.token);
			var encryptedToken = scrypt.crypto_scrypt(scrypt.encode_utf8(secret), scrypt.encode_utf8(data.token), 128, 8, 1, 32);
			var encryptedTokenhex = scrypt.to_hex(encryptedToken);
			//check the auth data sent by the client
		    	...
		    	if (err) {
		    		...
		    		var returnData = JSON.parse(userData);
								...
								returnData.projects.every(function(element, index, array) {
									if (element.id === data.project.id) {
									    console.log(index);
									    tempArray[index].status = 'complete';
									    return false;
									  } else{
									    return true;
									  };
									        ...
									        if (err) {
									            ...
									            client.expire(encryptedTokenhex, 86400, function(error, success) {
									                if (error) {
									                    ...
									                    //restricted.to(socket.id).emit('recieve blab', tempArray);
				        								console.log('project set to complete');	
									                }
									                else {
									                    console.log(new Error('Expiration not set on redis'));
									                }
		...
		socket.on('request profileUpdate', function(data){
			...
			client.get(encryptedTokenhex, function(err, userData) {
		    	if (err) {
		    		...
		    		returnData = JSON.parse(userData);
		    		console.log(returnData.username);
...
*/
		socket.on('request project', function(projectId) {
	
		  	//finds users in the database that have the same username already
			db.users.find({"projects.id":projectId},{"projects.$": 1}, function(err, project){
				if ( err || !project) { 
					console.log("DB error"); 
					restricted.to(socket.id).emit('request error');
				} else {
					if (project.length == 1) {
						restricted.to(socket.id).emit('recieve project', project);
					} else {
			...
			socket['currentRoomId'] = projectId;
			console.log('joined room: '+ socket['currentRoomId'] );
			socket.join(projectId+"Chat0");
			socket.join(projectId+"Chat1");
			socket.join(projectId+"Comment1");
		});

		socket.on('request projectUpdate', function(project) {
		  	//finds users in the database that have the same username already
			db.users.update(
								   {"projects.id":project.id},
								   {$set: {'projects.$.tasks': project.tasks, 'projects.$.components': project.components } }
						);
						console.log("project updated");
						//restricted.to(socket.id).emit('recieve project', project);
		});

		socket.on('request chatMsg', function(msgObj) {
			console.log(msgObj);
		  	//finds users in the database that have the same username already
			db.users.update({
				"projects.id" : msgObj.projectId
			},{
				$push: {'projects.$.chat' : msgObj }
			},{ 
				upsert: true 
			});
			console.log("push chatMsg to room: " + msgObj.projectId+"Chat0");
			restricted.to(msgObj.projectId+"Chat0").emit('recieve chatMsg', msgObj);
		});

		socket.on('request commentMsg', function(data) {
			...
			client.get(encryptedTokenhex, function(err, userData) {
		    	if (err) {
		    		...
		    		console.log(returnData.username);
					db.users.update({
						"projects.id" : data.msgObj.projectId
						},{
							$push: {'projects.$.comments' : data.msgObj }
						},{ 
							upsert: true 
					});
					var tempArray = returnData.projects;
								...
								returnData.projects.every(function(element, index, array) {
									if (element.id === data.msgObj.projectId) {
									    console.log(index);
									    tempArray[index].comments.push(data.msgObj);
									    return false;
									  } else{
									    return true;
									  };
						        ...
						        if (err) {
						            ...
						            client.expire(encryptedTokenhex, 86400, function(error, success) {
						                if (error) {
						                    ...
						                    restricted.to(data.msgObj.projectId+"Comment1").emit('recieve commentMsg', data.msgObj);
	        								console.log('profile updated');
						                }
						                else {
						                    console.log(new Error('Expiration not set on redis'));
